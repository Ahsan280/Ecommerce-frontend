...
const CartIcon = () => {
  ...
  return (
    <Link to="/shopping-cart" className="relative">
      ...
      <span className="absolute -top-3 -right-3  text-white text-xs font-bold rounded-full h-6 w-6 flex items-center justify-center">
        {cart?.items?.length > 0 ? cartLength() : 0}
      </span>
    </Link>
  );
};
